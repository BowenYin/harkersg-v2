...
exports.newUser=functions.firestore.document("users/{userId}").onCreate((snapshot, context)=>{
...
exports.addFolder=functions.https.onCall((data, context)=>{
  try {
    ...
    if (!context.auth.token.email.match(/.+(?:@students\.|@staff\.|@)harker.org$/))
      throw new functions.https.HttpsError("permission-denied", "Must use a school email address.");
    if (typeof data.title!="string" || data.title.length<4 || data.title.length>30)
      throw new functions.https.HttpsError("invalid-argument", "Please check data again.");
    return firestore.collection("users").doc(context.auth.uid).get().then(doc=>{
      ...
      return firestore.runTransaction(transaction=>{
        ...
        console.log('Folder "'+data.title+'" added to '+data.course+' by '+context.auth.token.name+' (UID '+context.auth.uid+').');
      });
    });
  } catch (error) {
    if (error instanceof functions.https.HttpsError) throw error;
